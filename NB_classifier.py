...
for row in cursor_dev:
    ...
    c = NBClassifier.classify(fetch_features(featureVector))
    if c == "Positive":
        nb_pos += 1.0
    else:
        nb_neg += 1.0
...
precision_pos = float(correct_pos_nb/nb_pos)
precision_neg = float(correct_neg_nb/nb_neg)
recall_pos = float(correct_pos_nb/count_pos)
recall_neg = float(correct_neg_nb/count_neg)
